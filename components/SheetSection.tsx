...
export default function SheetSection({ activeFile, onActivateSheet }: SheetSectionProps) {
  const [expandedSheets, setExpandedSheets] = useState<{ [key: string]: boolean }>({
    // Keep first sheet expanded by default, others collapsed
    ...
    <div className="w-full space-y-4">
      ...
      <div className="space-y-3">
        {activeFile.sheetNames.map((sheetName) => {
          ...
          // Local target column for preview chart
          const previewY = sheetData.numericColumns[0];
          const previewX = sheetData.categoricalColumns[0] || sheetData.columns[0];
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[9px] font-bold bg-slate-50 text-slate-500 border border-slate-100">
                      <Database size={10} />
                      {sheetData.data.length} Baris
                    </span>
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[9px] font-bold bg-slate-50 text-slate-500 border border-slate-100">
                      <BarChart3 size={10} />
                      {sheetData.numericColumns.length} Variabel
                    </span>
                  </div>
                </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div className="bg-slate-50/50 border border-slate-100/50 p-3.5 rounded-xl">
                          ...
                          <p className="text-xs font-bold text-slate-700 mt-1 truncate">
                            {sheetData.categoricalColumns.join(', ') || '-'}
                          </p>
                        </div>
                        <div className="bg-slate-50/50 border border-slate-100/50 p-3.5 rounded-xl">
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">
                      ...
                      {previewY && (
                        <div className="space-y-1.5">
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                            Tren Pratinjau ({previewY})
                          </span>
                          <div className="h-[180px] bg-slate-50/30 border border-slate-100 rounded-xl p-3">
                            <ResponsiveContainer width="100%" height="100%">
                              <AreaChart data={sheetData.data} margin={{ top: 10, right: 10, left: -20, bottom: -10 }}>
                                <defs>
                                ...
                                <XAxis dataKey={previewX} tick={{ fontSize: 9, fill: '#94A3B8' }} axisLine={false} tickLine={false} />
