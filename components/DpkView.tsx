...
import { downloadDpkTemplate } from '../services/excelService';
...
export default function DpkView({ activeFile }: DpkViewProps) {
  ...
  const [selectedCategories, setSelectedCategories] = useState<string[]>(['Giro', 'Tabungan', 'Deposito']);
  ...
  const rawSheet = useMemo(() => {
    ...
    // Default template data matching user's exact DPK screenshot
    return {
      ...
      data: [
        {
          indicator: 'Giro',
          '2024-Mei': 136614038441794,
          '2025-Mei': 149838852087387,
          '2026-Mei': 174830850692072,
          'YOY': 0.1668,
          'SHARE': 0.2290
        },
        {
          indicator: 'Tabungan',
          '2024-Mei': 318730159773855,
          '2025-Mei': 332539196995268,
          '2026-Mei': 361901013518981,
          'YOY': 0.0883,
          'SHARE': 0.4740
        },
        {
          indicator: 'Deposito',
          '2024-Mei': 239460401053625,
          '2025-Mei': 232504312637831,
          '2026-Mei': 226832053960849,
          'YOY': -0.0244,
          'SHARE': 0.2971
        },
        {
          indicator: 'Total',
          '2024-Mei': 694804599269274,
          '2025-Mei': 714882361720486,
          '2026-Mei': 763563918171902,
          'YOY': 0.0681,
          'SHARE': 1.0000
        }
      ]
  ...
  const availableCategories = ['Giro', 'Tabungan', 'Deposito'];
  ...
  const handleExportSVG = () => {
    const chartContainer = document.getElementById('dpk-chart-container');
    ...
    link.href = blobUrl;
    link.download = `FINSIGHT_DPK_Portofolio_${chartType}_${Date.now()}.svg`;
  ...
  const handleExportPNG = () => {
    ...
    img.onload = () => {
      ...
      const link = document.createElement('a');
      link.href = pngUrl;
      link.download = `FINSIGHT_DPK_Portofolio_${chartType}_${Date.now()}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  return (
    ...
    >
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-soft flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-400 font-medium">| DPK per Portofolio</span>
          </div>
          <h1 className="text-xl font-black text-slate-800 tracking-tight">
            Dana Pihak Ketiga (DPK) per Portofolio
          </h1>
          <p className="text-xs text-slate-500 font-medium mt-1">
            Visualisasi penghimpunan DPK (Giro, Tabungan, Deposito) dengan filter grafik interaktif, analisis YoY, dan komposisi Share (%).
          </p>
        </div>
        ...
        >
          ...
          <span>Download Template DPK</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {/* Giro Card */}
        {selectedCategories.includes('Giro') && (
          <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-soft relative overflow-hidden group hover:border-indigo-200 transition-colors">
            <div className="flex items-center justify-between mb-3">
              ...
              {showShare && (
                <span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">
                  Share: {(giroRow?.['SHARE'] ? (giroRow['SHARE'] * 100).toFixed(2) : '22.90')}%
                </span>
              )}
            </div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Giro ({effYoyTarget})</p>
            <h3 className="text-lg font-black text-slate-800 font-mono mt-1">
              {formatRupiahTrillion(giroRow?.[latestPeriodKey] || giroRow?.['2026-Mei'] || 174830850692072)}
            </h3>
            {showYoy && (
              <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-slate-50 text-xs font-semibold">
                <TrendingUp size={14} className="text-emerald-500" />
                <span className="text-emerald-600 font-bold">
                  {formatPercent(giroRow?.['YOY'] ?? 0.1668)}
                </span>
        ...
        {/* Tabungan Card */}
        {selectedCategories.includes('Tabungan') && (
          <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-soft relative overflow-hidden group hover:border-emerald-200 transition-colors">
            <div className="flex items-center justify-between mb-3">
              ...
              {showShare && (
                <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">
                  Share: {(tabunganRow?.['SHARE'] ? (tabunganRow['SHARE'] * 100).toFixed(2) : '47.40')}%
                </span>
              )}
            </div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Tabungan ({effYoyTarget})</p>
            <h3 className="text-lg font-black text-slate-800 font-mono mt-1">
              {formatRupiahTrillion(tabunganRow?.[latestPeriodKey] || tabunganRow?.['2026-Mei'] || 361901013518981)}
            </h3>
            {showYoy && (
              <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-slate-50 text-xs font-semibold">
                <TrendingUp size={14} className="text-emerald-500" />
                <span className="text-emerald-600 font-bold">
                  {formatPercent(tabunganRow?.['YOY'] ?? 0.0883)}
                </span>
                <span className="text-slate-400 font-normal">YoY</span>
              </div>
            )}
        ...
        {/* Deposito Card */}
        {selectedCategories.includes('Deposito') && (
          <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-soft relative overflow-hidden group hover:border-amber-200 transition-colors">
            <div className="flex items-center justify-between mb-3">
              ...
              {showShare && (
                <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">
                  Share: {(depositoRow?.['SHARE'] ? (depositoRow['SHARE'] * 100).toFixed(2) : '29.71')}%
                </span>
              )}
            </div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Deposito ({effYoyTarget})</p>
            <h3 className="text-lg font-black text-slate-800 font-mono mt-1">
              {formatRupiahTrillion(depositoRow?.[latestPeriodKey] || depositoRow?.['2026-Mei'] || 226832053960849)}
            </h3>
            {showYoy && (
              <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-slate-50 text-xs font-semibold">
                {Number(depositoRow?.['YOY']) < 0 ? (
                  <TrendingDown size={14} className="text-red-500" />
                ) : (
                  <TrendingUp size={14} className="text-emerald-500" />
                )}
                <span className={Number(depositoRow?.['YOY']) < 0 ? 'text-red-600 font-bold' : 'text-emerald-600 font-bold'}>
                  {formatPercent(depositoRow?.['YOY'] ?? -0.0244)}
                </span>
                <span className="text-slate-400 font-normal">YoY</span>
              </div>
            )}
        ...
        <div className="bg-gradient-to-br from-[#C61E1E] to-[#9E1818] text-white rounded-2xl p-5 shadow-lg relative overflow-hidden">
          ...
          <p className="text-xs font-bold text-red-200 uppercase tracking-wider">Total Penghimpunan DPK</p>
          <h3 className="text-lg font-black text-white font-mono mt-1">
            {formatRupiahTrillion(totalRow?.[latestPeriodKey] || totalRow?.['2026-Mei'] || 763563918171902)}
          </h3>
          {showYoy && (
            <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-white/20 text-xs font-semibold text-red-100">
              ...
              <span className="text-white font-bold">
                {formatPercent(totalRow?.['YOY'] ?? 0.0681)}
              </span>
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-soft space-y-4">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between border-b border-slate-50 pb-4 gap-3">
          <div>
            <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
              ...
              <span>Tren Nominal DPK per Portofolio (Triliun Rp)</span>
            </h3>
        ...
        <div id="dpk-chart-container" className="h-80 w-full relative">
      ...
      {/* DEDICATED STANDALONE VISUALIZATION SECTION: YOY & SHARE WITH CLEAR EXPLICIT YEAR LABELS */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        
        {/* Standalone Visualization 1: Grafik Pertumbuhan YoY DPK */}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-50 pb-3 gap-3">
              <div>
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                  ...
                  <span>Visualisasi Pertumbuhan YoY DPK</span>
                ...
                >
                  ...
                  <Tooltip 
                    formatter={(val: any, name: any, item: any) => [
                      ...
                      `Pertumbuhan YoY DPK (${effYoyBase} ➔ ${effYoyTarget})`
                    ]}
                    contentStyle={{ backgroundColor: '#1E293B', borderRadius: '12px', color: '#FFF', fontSize: '12px' }}
                  />
        ...
        {/* Standalone Visualization 2: Grafik Distribusi Pangsa Pasar & Share (%) DPK */}
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-soft space-y-4 flex flex-col justify-between">
          <div>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-50 pb-3 gap-2">
              <div>
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                  ...
                  <span>Visualisasi Distribusi Share DPK (%)</span>
                </h3>
                <p className="text-[11px] text-slate-400 font-medium">
                  Proporsi kontribusi penghimpunan DPK per portofolio
                </p>
              </div>
            ...
            <div className="space-y-3 my-5">
              <div className="flex items-center justify-between text-xs font-bold text-slate-600">
                <span>Distribusi Portofolio DPK ({effShareYear})</span>
          ...
          <div className="p-3 bg-blue-50/50 border border-blue-100 rounded-2xl flex items-center justify-between text-xs text-blue-900 font-semibold">
            <span>Portofolio DPK Terbesar ({effShareYear}):</span>
      ...
      {/* Dynamic Master Table View with Filter Toggles matching exact user screenshot */}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-100 pb-4 gap-2">
          <div>
            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">
              Struktur Tabel Template Excel DPK (Filter Aktif)
            </h3>
            <p className="text-[11px] text-slate-400 font-semibold mt-0.5">
              Menampilkan {activePeriods.length} periode & {filteredData.mainRows.length} portofolio DPK terpilih
            </p>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full">
              Format Standar OJK DPK
            </span>
          </div>
        </div>
                ...
                <tr className="bg-slate-100/70 font-black text-[#1E293B] border-t-2 border-t-slate-300 border-b border-slate-200">
                  ...
                  {showYoy && (
                    <td className="p-3.5 border-r border-slate-200 text-right font-mono font-black text-emerald-700 bg-emerald-50/50">
                      {formatPercent(filteredData.totalRow?.['YOY'] ?? 0.0681)}
                    </td>
                  )}
                  {showShare && (
